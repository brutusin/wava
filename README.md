...
### Memory promises
Each job is submitted with a promise of maximum RSS usage. The scheduler keeps track of these needs in order to guarantee that the set of running jobs never exceed a (configurable) amount of total physical memory.
Of course promises might be wrong, so the the scheduler periodically verifies them for the running jobs, and in case of failing, a (configurable) action is performed, by default: killing the job process tree.
